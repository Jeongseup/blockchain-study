...
func (ddb debugDB) Get(key []byte) (value []byte) {
	defer func() {
		fmt.Printf("%v.Get(%v) %v\n", ddb.label,
			common.ColoredBytes(key, common.Cyan, common.Blue),
			common.ColoredBytes(value, common.Green, common.Blue))
	}()
...
func (ddb debugDB) Has(key []byte) (has bool) {
	defer func() {
		fmt.Printf("%v.Has(%v) %v\n", ddb.label,
			common.ColoredBytes(key, common.Cyan, common.Blue), has)
	}()
...
func (ddb debugDB) Set(key []byte, value []byte) {
	fmt.Printf("%v.Set(%v, %v)\n", ddb.label,
		common.ColoredBytes(key, common.Yellow, common.Blue),
		common.ColoredBytes(value, common.Green, common.Blue))
...
func (ddb debugDB) Delete(key []byte) {
	fmt.Printf("%v.Delete(%v)\n", ddb.label,
		common.ColoredBytes(key, common.Red, common.Yellow))
	ddb.db.Delete(key)
}
...
func (ddb debugDB) Iterator(start, end []byte) Iterator {
	fmt.Printf("%v.Iterator(%v, %v)\n", ddb.label,
		common.ColoredBytes(start, common.Cyan, common.Blue),
		common.ColoredBytes(end, common.Cyan, common.Blue))
